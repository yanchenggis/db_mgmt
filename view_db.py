    ...
    if analysis['unique_fields']:
        ...
        for field in analysis['unique_fields']:
            ...
            if 'min' in field:
                ...
                print(f"  Sum: {field['sum']:.2f}" if isinstance(field['sum'], float) else f"  Sum: {field['sum']}")
                print(f"  Std Dev: {field['std']:.2f}" if isinstance(field['std'], float) else f"  Std Dev: {field['std']}")
